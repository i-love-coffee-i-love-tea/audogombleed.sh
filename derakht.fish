...
# Guard: reject direct execution (like derakht.sh does under bash/zsh)
if test "$__CLI_PROGNAME" = "derakht.fish"
    echo "This script is not intended to be called directly." >&2
    echo "Create a wrapper script and an alias with the same name." >&2
    echo >&2
    echo "    # once: create a config file and a fish wrapper" >&2
    echo "    touch ~/.yourcli.conf" >&2
    echo "    echo '#!/usr/bin/env fish' > ~/bin/yourcli" >&2
    echo "    echo 'set -g __CLI_PROGNAME yourcli' >> ~/bin/yourcli" >&2
    echo "    echo 'set -g __cli_wrapper_argv \$argv' >> ~/bin/yourcli" >&2
    echo "    echo 'source ~/bin/derakht.fish' >> ~/bin/yourcli" >&2
    echo "    chmod +x ~/bin/yourcli" >&2
    exit 49
end
