...
@test "fish: _awk output=command_names returns commands" {
    run _fish_eval 'count (_awk output=command_names)'
    assert_success
    [ "$output" -gt 0 ]
}

@test "fish: _awk output=commands returns command list" {
    run _fish_eval 'count (_awk output=commands)'
    assert_success
    [ "$output" -gt 0 ]
}

@test "fish: _awk output=help returns help text" {
    run _fish_eval 'count (_awk output=help do_format=1)'
    assert_success
    [ "$output" -gt 0 ]
}

@test "fish: _awk output=env returns env lines" {
    run _fish_eval 'count (_awk output=env)'
    assert_success
    [ "$output" -gt 0 ]
}
